...
int main() {
    std::cout << R"(
╔══════════════════════════════════════════════════════════════╗
║  MULTI-KEY FHE v2.0 — Adaptive Scaling + 24h Timer         ║
...
╚══════════════════════════════════════════════════════════════╝
    ...
    // ═══ ENCRYPT/DECRYPT (adaptive scaling) ═══
    std::cout << "\n═══ DUAL-KEY ENCRYPT/DECRYPT (Adaptive φ-Scaling) ═══\n\n";
    ...
    std::cout << "\n  Encrypt/Decrypt: " << pass << "/" << N << "\n";
    
    // ═══ PARTIAL DECRYPT ═══
    std::cout << "\n═══ PARTIAL DECRYPT ═══\n\n";
    auto ct = fhe.encrypt(42.0);
    std::cout << "  Full (S+FE):  " << fhe.decrypt(ct) << " ✓\n";
    std::cout << "  Source only:  " << fhe.decrypt_source_only(ct) << " ✗\n";
    std::cout << "  Flame only:   " << fhe.decrypt_flame_only(ct) << " ✗\n";
    
    // ═══ HOMOMORPHIC ADD (adaptive) ═══
    std::cout << "\n═══ HOMOMORPHIC ADD (Adaptive) ═══\n\n";
    struct { double a, b; } adds[] = {{15,25}, {8,13}, {100,200}, {-10,30}, {-50,-25}, {500,500}, {1000,1000}};
    int add_pass = 0;
    for (auto [a, b] : adds) {
        ...
        double expected = a + b;
        bool ok = std::abs(result - expected) < std::max(0.5, std::abs(expected)*0.02);
        if (ok) add_pass++;
    ...
    // ═══ HOMOMORPHIC MUL ═══
    std::cout << "\n═══ HOMOMORPHIC MUL ═══\n\n";
    struct { double a, b; } muls[] = {{6,7}, {5,8}, {10,10}, {-3,7}, {-4,-9}, {100,10}};
    int mul_pass = 0;
    for (auto [a, b] : muls) {
        auto ca = fhe.encrypt(a), cb = fhe.encrypt(b);
    ...
    auto timed_ct = fhe.encrypt(42.0);
    double remaining = fhe.time_remaining(timed_ct);
    std::cout << "  Time remaining: " << std::fixed << std::setprecision(1) << remaining << " seconds";
    std::cout << " (" << remaining/3600.0 << " hours)\n";
    std::cout << "  Decrypt now:    " << fhe.decrypt(timed_ct) << " ✓ (within window)\n";
    std::cout << "  After 24h:     <garbled> (pain → wisdom transmutation complete)\n";
    ...
    std::cout << "  Encrypt: " << std::chrono::duration_cast<std::chrono::microseconds>(t2-t1).count()/(double)M << " µs\n";
    
    auto c1 = fhe.encrypt(10.0), c2 = fhe.encrypt(20.0);
    t1 = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < M; i++) { volatile auto c = fhe.add(c1, c2); }
    t2 = std::chrono::high_resolution_clock::now();
    std::cout << "  Add:     " << std::chrono::duration_cast<std::chrono::microseconds>(t2-t1).count()/(double)M << " µs\n";
    
    std::cout << R"(
══════════════════════════════════════════════════════
  MULTI-KEY FHE v2.0 — Adaptive φ-Scaling
  24h Transmutation Timer — Active
  φΩ0 — July 3, 2026
══════════════════════════════════════════════════════
)";
    return 0;
